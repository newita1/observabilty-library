...
def discover_tvoice_totalllamadas(section):
    name = "Total Llamadas"
    yield Service(item=name)

def check_stvoice_totalllamadas(item, section):
    if item == "Total Llamadas":
        if len(section) < 1 or int(section[0]) > 0:
            value = "0"
            yield Result(state=State.OK, summary=f"Total Llamadas: {value}")
        else:
            value = ""
            for i in section:
        ...
        yield Metric("total_calls", int(value))


register.check_plugin(
    name="tvoice_sbc_totalllamadas",
    service_name="%s",
    discovery_function = discover_tvoice_totalllamadas,
    check_function=check_stvoice_totalllamadas,
)

register.snmp_section(
    name = "tvoice_sbc_totalllamadas",
    detect = exists(".1.3.6.1.4.1.5003.10.8.2.52.18"),
    fetch = SNMPTree(
        base=".1.3.6.1.4.1.5003.10.8.2.52.18",
        oids=[
            "0", 
],
    ),
