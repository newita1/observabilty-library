...
def discover_tvoice_llamadasactivas(section):
    name = "Llamadas Activas"
    yield Service(item=name)

def check_stvoice_llamadasactivas(item, section):
    if item == "Llamadas Activas":
        if len(section) < 1 or int(section[0]) > 0:
            value = "0"
            yield Result(state=State.OK, summary=f"Llamadas Activas: {value}")
        else:
            value = ""
            for i in section:
        ...
        yield Metric("active_calls", int(value))


register.check_plugin(
    name="tvoice_sbc_llamadasactivas",
    service_name="%s",
    discovery_function = discover_tvoice_llamadasactivas,
    check_function=check_stvoice_llamadasactivas,
)

register.snmp_section(
    name = "tvoice_sbc_llamadasactivas",
    detect = exists(".1.3.6.1.4.1.5003.10.8.2.52.41"),
    fetch = SNMPTree(
        base=".1.3.6.1.4.1.5003.10.8.2.52.41",
        oids=[
            "0", 
],
    ),
